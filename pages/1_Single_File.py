...
st.set_page_config(page_icon=":wave:",layout="wide") # basic page configuration
st.title("Hello Sir!!! :wave:")
st.header("Welcome to single file page. :v:")
st.write("If you want to convert a single input file into output file, then you are on the correct page. :heavy_check_mark:")
...
def Octant_Processing(m,mod=5000):
	...
	try: # opening the input excel sheet
		os.chdir(r'C:\Users\User\Documents\GitHub\Octant_Processing\input') # making the parent directory as input directory
	...
	try: # opening the input excel sheet
		os.chdir(r'C:\Users\User\Documents\GitHub\Octant_Processing\output') # making the parent directory as input directory
		output=op.Workbook()
...
st.subheader("1. Uploading file method:")
input_files=st.file_uploader("Upload the input file",type=["xlsx"], accept_multiple_files=False)

st.write("##")

st.subheader("2. Taking complete path of the file method:")
input_path=st.text_input("Upload the path of the input files",placeholder=r"Please enter the path, for example:- 'C:\Users\User\Documents\GitHub\Octant_Processing\input\1.0.xlsx' (without inverted commas)")
...
# location=st.text_input("If you want to download the files in a specific path then please state, otherwise it will get saved in output_files folder in project 2:",placeholder=r"Please enter the path, for example:- 'C:\Users\User\Documents\GitHub\Octant_Processing\input' (without inverted commas)",value="")

if work:
	try:
		os.mkdir(r'C:\Users\User\Documents\GitHub\Octant_Processing\output')
	except:
		pass
	
	Octant_Processing(input_files,mod)
	
	try:
		if input_path[-5:]==".xlsx":
			dirname, fname = os.path.split(input_path)
			Octant_Processing(fname,mod)
	except:
		if input_path!="":
